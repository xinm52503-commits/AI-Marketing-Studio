...
- 产品形态：桌面端优先的 Web App
- 目标市场：Amazon US
- 输出语言：营销素材为英文；产品界面优先中文
- 目标读者：产品、设计、前端、后端、AI 工程与测试人员

## 2. 产品概述

### 2.1 一句话定位

面向中小型 Amazon US 跨境电商卖家，以真实商品资料为唯一事实源，生成可追溯、可审批的商品详情页图片和短视频素材。

### 2.2 用户问题

卖家可以直接向通用 AI 输入商品资料，但仍需自行组织上下文、反复校对事实、在多个生成工具之间复制内容，并管理不同素材版本。长对话还可能混用旧参数或产生未经验证的宣传声明。

本产品不以“生成提示词”为核心价值，而是提供从商品事实、创意规划、人工审批到图片和视频生成的完整工作流。

### 2.3 产品目标

- 将真实商品资料转化为一套统一的 Amazon 详情页创意方案。
- 让事实型营销文案可追溯到用户提供的商品事实。
- 在图片和视频生成前设置人工审批门槛。
- 按 Creative Brief → 图片 → 视频的顺序控制生成成本。
- 在单项生成失败时保留其他已完成结果。

### 2.4 非目标

- 不接入真实 Amazon 发布 API。
- 不承诺完成全部 Amazon 政策审核，仅提供主图建议和基础风险提示。
- 不实现登录、支付、数据库、项目历史和团队权限。
- 不实现多 Agent、Critic Agent、自动优化循环和复杂状态机。
- 不生成完整 Listing 标题、五点描述或销售分析。
- 不支持多平台、多站点和批量商品。
- 不宣称未经过真实测试的 ROI、转化率、用户数或效果数据。

## 3. 目标用户与使用场景

### 3.1 核心用户

- 中小型 Amazon US 卖家：有真实商品资料，但缺少专业设计、英文广告文案和生成模型使用经验。
- 电商运营人员：需要为单个商品快速规划并审核整套详情页视觉素材。

### 3.2 潜在协作角色

- 商品负责人：维护商品事实并确认其真实性。
- 运营审核者：选择卖点、修改文案并批准生成。
- 设计师：将生成结果作为初稿继续加工。

V1 不实现角色权限；以上角色可以由同一用户承担。

### 3.3 典型场景

1. 新品准备：卖家输入新商品参数和参考图，希望获得统一的主图建议、三张副图和短视频方案。
2. 素材改版：运营人员修改目标受众或核心卖点，仅重新生成受影响的单张素材。
3. 风险校对：用户输入“保温 24 小时”等无证据声明，系统阻止其进入最终生成提示。

### 3.4 待验证假设

- 卖家认为事实引用带来的可控性高于额外审核成本。
- 用户愿意先批准 Creative Brief，再等待图片生成。
- 用户在确认图片方向后，愿意为视频生成消્ઙ江/not sure?
